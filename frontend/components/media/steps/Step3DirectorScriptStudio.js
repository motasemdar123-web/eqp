      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`flex items-center justify-between text-[10px] pt-0.5 ${isSelected ? 'text-slate-400' : 'text-slate-500'}`}>
                      ...
                      <span>{isScripted ? `${c.scenes?.length || c.slides?.length} scenes` : 'Pending Script'}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="lg:col-span-8 space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          ...
          <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-200 pb-2.5 text-xs font-medium">
            ...
            >
              B-Roll Checklist ({scriptData.brollChecklist?.length || 0})
            </button>
          ...
          {activeTab === 'scenes' && (
            <div className="space-y-3.5">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={handleAddScene} className="text-xs !bg-slate-100 font-semibold">
                  + Add Scene
                </Button>
              </div>
          ...
          {activeTab === 'hook' && (
            <div className="space-y-4">
              <div className="bg-amber-50/70 p-3.5 rounded-lg border border-amber-200 text-xs text-amber-900 space-y-1">
                <span className="font-bold uppercase tracking-wider block text-[11px]">Director Hook Formula (0–3s):</span>
                <p className="leading-relaxed text-amber-800 text-[11px]">
                  In heavy machinery B2B media, viewers decide in under 3 seconds. Combine a powerful visual grab (e.g. bucket cutting through rock at high speed), an intense mechanical sound, and a provocative spoken statement.
                </p>
              </div>

              <div>
                <label className="text-xs font-semibold text-slate-700 mb-1 block">Spoken Hook (English)</label>
                <input
                  type="text"
              ...
              <div dir="rtl">
                <label className="text-xs font-semibold text-slate-700 mb-1 block text-right">النص الصوتي للثواني الأولى (عربي)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 mb-1 block">Visual Action Grabber (Camera movement)</label>
                <input
                  type="text"
