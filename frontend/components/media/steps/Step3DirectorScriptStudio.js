...
export default function Step3DirectorScriptStudio({
  ...
  const handleCopyOnAssetText = () => {
    const text = `🎨 DESIGNER INSTRUCTIONS & TOV
===================================================
POST #${scriptData.conceptNumber || ''}: ${scriptData.title || ''}
FORMAT: ${scriptData.format?.toUpperCase() || ''} | PILLAR: ${pillarMeta?.label || ''}
---------------------------------------------------
🗣️ TONE OF VOICE (TOV):
${scriptData.tov || 'Authoritative, Japanese Precision, Industrial Strength'}

🇬🇧 TEXT ON PIC / VIDEO (ENGLISH):
${scriptData.visualHeadlineEn !== undefined ? scriptData.visualHeadlineEn : onAssetCopyData.headlineEn}

🇰🇼 TEXT ON PIC / VIDEO (ARABIC):
${scriptData.visualHeadlineAr !== undefined ? scriptData.visualHeadlineAr : onAssetCopyData.headlineAr}

📝 DESIGNER NOTE / DIRECTIVE:
${scriptData.visualNotes !== undefined ? scriptData.visualNotes : onAssetCopyData.designNotes}
===================================================`;
    ...
    <div className="space-y-6 animate-[ds-toast-in_180ms_ease]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4 bg-white p-5 rounded-xl border border-slate-200 shadow-2xs">
          ...
          {/* ========================================================= */}
          {/* DESIGNER TOV & VISUAL COPY SECTION (DIRECTLY EDITABLE)    */}
          {/* ========================================================= */}
          <div className="bg-amber-50/60 border border-amber-200/90 rounded-xl p-4 space-y-3 shadow-2xs">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-amber-200/80 pb-2">
              <div className="flex items-center gap-2">
                <span className="text-base">🎨</span>
                <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
                  Designer Instructions & Tone of Voice (Text on Pic / Video)
                </h4>
              </div>
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-[11px] bg-slate-900 hover:bg-slate-800 text-white font-bold px-2.5 py-1 rounded cursor-pointer transition-colors shadow-2xs"
                >
                  {copiedOnAsset ? '✓ Copied!' : '📋 Copy for Designer'}
                </button>
                <span className="text-[11px] text-amber-900 font-medium">✏️ Directly editable • Auto-saved</span>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {/* Text on Pic/Video (EN) */}
              <div>
                <label className="text-xs font-bold text-slate-800 mb-1 block">
                  🇬🇧 Text to Write on Pic / Video (English)
                </label>
                <input
                  type="text"
                  value={scriptData.visualHeadlineEn !== undefined ? scriptData.visualHeadlineEn : (onAssetCopyData.headlineEn || '')}
                  onChange={(e) => {
                    handleFieldChange('visualHeadlineEn', e.target.value);
                    handleOnAssetCopyChange('headlineEn', e.target.value);
                  }}
                  placeholder="e.g. CONQUERING 52°C DESERT HEAT"
                  className="ds-input text-xs font-bold text-slate-900 bg-white"
                />
              </div>

              {/* Text on Pic/Video (AR) */}
              <div dir="rtl">
                <label className="text-xs font-bold text-slate-800 mb-1 block text-right">
                  🇰🇼 النص على الصورة أو الفيديو (عربي)
                </label>
                <input
                  type="text"
                  value={scriptData.visualHeadlineAr !== undefined ? scriptData.visualHeadlineAr : (onAssetCopyData.headlineAr || '')}
                  onChange={(e) => {
                    handleFieldChange('visualHeadlineAr', e.target.value);
                    handleOnAssetCopyChange('headlineAr', e.target.value);
                  }}
                  placeholder="مثال: قهر حرارة الصحراء فوق 50 درجة مئوية"
                  className="ds-input text-xs font-bold text-slate-900 bg-white text-right"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={scriptData.tov || ''}
                  onChange={(e) => handleFieldChange('tov', e.target.value)}
                  placeholder="e.g. Authoritative, Japanese Precision, Industrial Strength"
                  className="ds-input text-xs font-semibold text-slate-900 bg-white"
                />
              </div>
              ...
              <div>
                <label className="text-xs font-bold text-slate-800 mb-1 block">
                  📝 Note for Designer (Style, Font, Placement)
                </label>
                <input
                  type="text"
                  value={scriptData.visualNotes !== undefined ? scriptData.visualNotes : (onAssetCopyData.designNotes || '')}
                  onChange={(e) => {
                    handleFieldChange('visualNotes', e.target.value);
                    handleOnAssetCopyChange('designNotes', e.target.value);
                  }}
                  placeholder="e.g. Bold DIN font, keep bottom-left, Komatsu yellow accent"
                  className="ds-input text-xs text-slate-900 bg-white"
                />
              </div>
            </div>
          </div>
