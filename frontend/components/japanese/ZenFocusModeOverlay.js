...
export default function ZenFocusModeOverlay({
  ...
  return (
    <div
      className={
        isOpen
          ? `fixed inset-0 z-50 flex flex-col overflow-hidden animate-fadeIn transition-colors duration-300 ${currentTheme.bg} ${currentTheme.text}`
          : 'w-full'
      }
    >
      {/* Top Floating Zen Control Bar (Rendered only when Focus Mode is open) */}
      {isOpen && (
        <header
        ...
        >
          ...
          <div className="flex items-center gap-2 bg-black/20 px-3 py-1.5 rounded-xl border border-white/10">
            <div className="flex items-center gap-1 text-[11px] font-bold opacity-75">
              <button
                ...
                className={`px-2 py-0.5 rounded cursor-pointer ${timerMode === 'pomodoro' ? 'bg-amber-500 text-slate-950 font-black' : 'hover:opacity-100'}`}
              >
                25m
              </button>
              <button
                ...
                className={`px-2 py-0.5 rounded cursor-pointer ${timerMode === 'short_break' ? 'bg-emerald-500 text-slate-950 font-black' : 'hover:opacity-100'}`}
              >
                5m Break
              </button>
              <button
                ...
                className={`px-2 py-0.5 rounded cursor-pointer ${timerMode === 'stopwatch' ? 'bg-blue-500 text-white font-black' : 'hover:opacity-100'}`}
              >
                ⏱
              </button>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={onClose}
              className={`font-bold text-xs flex items-center gap-1.5 active:scale-95 cursor-pointer ${
                themeId === 'paper'
      ...
      <main
        className={
          isOpen
            ? 'flex-1 overflow-y-auto p-3 sm:p-6 max-w-4xl mx-auto w-full flex flex-col justify-start items-center'
            : 'w-full'
        }
      >
        <div key="persistent-focus-root" className={isOpen ? 'w-full my-auto' : 'w-full'}>
          {children}
        </div>
      </main>
    </div>
