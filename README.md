...
EQP report generation still populates the existing Excel templates first, then exports the workbook result as PDF before uploading to Supabase. Docker installs LibreOffice for exact workbook-to-PDF conversion. If `LIBREOFFICE_BIN` or a system `soffice` binary is unavailable, the backend falls back to a local HTML-to-PDF renderer using `prince`/`princexml` when available, then to a built-in Node/PDFKit renderer. No paid external conversion API is required.

On Render, the best self-contained setup is a Docker service. The repository includes `render.yaml` and Dockerfiles; the Docker image installs LibreOffice and exposes `soffice` for conversion. If Docker is not possible on the current service, Render native deploys can use the local `prince` fallback.
