...
const workspaceNoteTypes = new Set(['note', 'knowledge']);
const workspaceNoteStatuses = new Set(['draft', 'active', 'archived']);
const workspaceVisibility = new Set(['private', 'team']);
const workspaceTaskStatuses = new Set(['todo', 'in_progress', 'done', 'blocked']);
const workspacePriorities = new Set(['low', 'medium', 'high', 'critical']);

const workspaceTemplateSeeds = [
  {
    title: 'Daily Plan',
    category: 'Planning',
    description: 'Plan today by priorities, site constraints, blockers, and follow-up actions.',
    tags: ['daily-plan', 'planning'],
    content: {
      blocks: [
        { type: 'heading', text: 'Daily Plan' },
        { type: 'checklist', text: 'Review open tasks and safety constraints' },
        { type: 'checklist', text: 'Confirm machine priorities and technician availability' },
        { type: 'callout', text: 'Risks / blockers:' },
      ],
    },
  },
  {
    ...
    category: 'Operations',
    description: 'Capture what changed, what remains open, and what the next shift must watch.',
    tags: ['handover', 'shift'],
    content: {
      blocks: [
        { type: 'heading', text: 'Shift Handover' },
        { type: 'paragraph', text: 'Completed work:' },
        { type: 'paragraph', text: 'Open issues:' },
        { type: 'paragraph', text: 'Parts, tools, or safety notes:' },
      ],
    },
  },
  {
    ...
    description: 'Structure symptoms, inspection evidence, hypothesis, action, and verification.',
    tags: ['fault', 'troubleshooting'],
    content: {
      blocks: [
        { type: 'heading', text: 'Fault Investigation' },
        { type: 'paragraph', text: 'Observed symptoms:' },
        { type: 'bullet', text: 'Initial checks performed' },
        { type: 'callout', text: 'Evidence and measurements:' },
        { type: 'paragraph', text: 'Recommended action:' },
      ],
    },
  },
  {
    ...
    description: 'Document problem statement, root cause, corrective action, and prevention.',
    tags: ['rca', 'improvement'],
    content: {
      blocks: [
        { type: 'heading', text: 'Root Cause Analysis' },
        { type: 'paragraph', text: 'Problem statement:' },
        { type: 'paragraph', text: 'Root cause:' },
        { type: 'paragraph', text: 'Corrective action:' },
        { type: 'paragraph', text: 'Preventive action:' },
      ],
    },
  },
  {
    ...
    category: 'Inspection',
    description: 'Reusable inspection checklist for machine condition and service readiness.',
    tags: ['inspection', 'checklist'],
    content: {
      blocks: [
        { type: 'heading', text: 'Machine Inspection Checklist' },
        { type: 'checklist', text: 'Inspect fluid levels and leaks' },
        { type: 'checklist', text: 'Check electrical connectors and battery condition' },
        { type: 'checklist', text: 'Record SMR, machine number, and observations' },
      ],
    },
  },
  {
    ...
    category: 'Meetings',
    description: 'Agenda, decisions, owners, due dates, and unresolved points.',
    tags: ['meeting', 'notes'],
    content: {
      blocks: [
        { type: 'heading', text: 'Maintenance Meeting Notes' },
        { type: 'paragraph', text: 'Agenda:' },
        { type: 'paragraph', text: 'Decisions:' },
        { type: 'checklist', text: 'Action item / owner / due date' },
      ],
    },
  },
  {
    ...
    description: 'Capture maintenance process improvement ideas and expected impact.',
    tags: ['idea', 'kaizen'],
    content: {
      blocks: [
        { type: 'heading', text: 'Improvement Idea' },
        { type: 'paragraph', text: 'Current issue:' },
        { type: 'paragraph', text: 'Suggested improvement:' },
        { type: 'paragraph', text: 'Expected impact:' },
      ],
    },
  },
  {
    ...
    category: 'Safety',
    description: 'Document hazard, immediate action, risk level, and preventive control.',
    tags: ['safety', 'observation'],
    content: {
      blocks: [
        { type: 'heading', text: 'Safety Observation' },
        { type: 'paragraph', text: 'Observation:' },
        { type: 'paragraph', text: 'Immediate action:' },
        { type: 'paragraph', text: 'Preventive control:' },
      ],
    },
  },
];
...
function workspaceVisibleWhere(actor) {
...
function workspaceTaskVisibleWhere(actor) {
  if (isWorkspaceAdmin(actor)) {
    return { deletedAt: null };
  }
...
function normalizeWorkspaceTags(value) {
  if (!value) return [];
...
function normalizeWorkspaceContent(content) {
  ...
  return {
    blocks: blocks.slice(0, 80).map((block) => ({
      type: ['heading', 'paragraph', 'bullet', 'checklist', 'callout', 'divider', 'table'].includes(block.type)
        ? block.type
        : 'paragraph',
...
function workspaceContentExcerpt(content) {
  const blocks = Array.isArray(content?.blocks) ? content.blocks : [];
  return blocks
...
function normalizeWorkspaceNote(note) {
  return {
    ...
    excerpt: workspaceContentExcerpt(note.content),
    type: note.type,
    status: note.status,
    tags: normalizeWorkspaceTags(note.tags),
    category: note.category,
    isPinned: note.isPinned,
    visibility: note.visibility,
    linkedMachineId: note.linkedMachineId,
    linkedTechnicianId: note.linkedTechnicianId,
    linkedReportId: note.linkedReportId,
...
function normalizeWorkspaceTask(task) {
  return {
    ...
    tags: normalizeWorkspaceTags(task.tags),
...
function normalizeWorkspaceTemplate(template) {
  return {
    ...
    tags: normalizeWorkspaceTags(template.tags),
...
function normalizeWorkspaceActivity(activity) {
  return {
    id: activity.id,
...
async function logWorkspaceActivity(prisma, actor, action, entityType, entityId) {
...
async function ensureWorkspaceTemplates(prisma) {
  const systemTemplateCount = await prisma.workspaceTemplate.count({
    where: { isSystem: true, deletedAt: null },
  });
  if (systemTemplateCount > 0) return;

  await prisma.workspaceTemplate.createMany({
    data: workspaceTemplateSeeds.map((template) => ({
...
async function ensureWorkspaceDemoContent(prisma, actor) {
  if (!actor?.sub) return;

  await ensureWorkspaceTemplates(prisma);

  const existingNotes = await prisma.workspaceNote.count({
    where: {
      createdById: actor.sub,
      deletedAt: null,
    },
  });
  const existingTasks = await prisma.workspaceTask.count({
    where: {
      createdById: actor.sub,
      deletedAt: null,
    },
  ...
  if (existingNotes === 0) {
    const sampleNotes = [
      ['Hydraulic overheating investigation', 'Troubleshooting', ['hydraulic', 'temperature'], 'Track oil temperature spikes, cooling fan behavior, and any restriction in radiator airflow.'],
      ['Shift handover notes', 'Operations', ['handover', 'shift'], 'Open observations for next shift: confirm machine availability and parts pending status.'],
      ['Generator cooling system observations', 'Machines', ['generator', 'cooling'], 'Document coolant level, radiator condition, hose pressure, and abnormal vibration.'],
      ['Preventive maintenance improvement ideas', 'Improvement', ['pm', 'improvement'], 'Reduce repeat checks by adding a standard inspection sequence before report generation.'],
    ];
    await prisma.workspaceNote.createMany({
      data: sampleNotes.map(([title, category, tags, text]) => ({
        id: crypto.randomUUID(),
        title,
        category,
        tags,
        ...
        status: 'active',
        visibility: 'private',
        isPinned: title === 'Hydraulic overheating investigation',
    ...
    const sampleKnowledge = [
      ['Standard hydraulic inspection checklist', 'SOP', ['hydraulic', 'inspection'], 'Check oil level, leaks, temperature, filters, fan belt condition, and abnormal noise.'],
      ['Common electrical fault troubleshooting', 'Troubleshooting', ['electrical', 'fault'], 'Start with battery voltage, grounds, fuses, relays, harness condition, and connector corrosion.'],
      ['Safety procedure for pressure testing', 'Safety', ['safety', 'pressure'], 'Depressurize, isolate, wear PPE, and verify gauge rating before pressure test.'],
      ['Shift handover best practices', 'Operations', ['handover', 'team'], 'Record completed work, blockers, parts, safety risks, and next shift priorities.'],
    ];
    await prisma.workspaceNote.createMany({
      data: sampleKnowledge.map(([title, category, tags, text]) => ({
        id: crypto.randomUUID(),
        title,
        category,
        tags,
        ...
        status: 'active',
        visibility: 'team',
        isPinned: title === 'Standard hydraulic inspection checklist',
        content: { blocks: [{ type: 'paragraph', text }] },
        createdById: actor.sub,
      })),
    });
  ...
  if (existingTasks === 0) {
    ...
    const sampleTasks = [
      ['Inspect hydraulic oil temperature on Machine D155A', 'in_progress', 'high', today, ['hydraulic', 'D155A']],
      ['Review EQP report for compressor issue', 'todo', 'medium', today, ['eqp', 'compressor']],
      ['Prepare tomorrow\'s maintenance plan', 'todo', 'medium', tomorrow, ['planning']],
      ['Update machine inspection checklist', 'blocked', 'low', tomorrow, ['checklist']],
    ];
    await prisma.workspaceTask.createMany({
      data: sampleTasks.map(([title, status, priority, dueDate, tags]) => ({
        id: crypto.randomUUID(),
        title,
        status,
        priority,
...
async function listWorkspaceSummary(actor) {
  const prisma = requirePrisma();
  await ensureWorkspaceDemoContent(prisma, actor);

  const noteWhere = workspaceVisibleWhere(actor);
  const taskWhere = workspaceTaskVisibleWhere(actor);
  ...
  const [
    ...
    openTasksCount,
    completedTodayCount,
    pinnedKnowledgeCount,
    criticalItemsCount,
  ] = await Promise.all([
    prisma.workspaceNote.findMany({
      where: { ...noteWhere, type: 'note', status: { not: 'archived' } },
      include: { createdBy: { select: publicUserSelect } },
      orderBy: { updatedAt: 'desc' },
      take: 5,
    }),
    prisma.workspaceNote.findMany({
      where: { ...noteWhere, type: 'note', isPinned: true, status: { not: 'archived' } },
      include: { createdBy: { select: publicUserSelect } },
      orderBy: { updatedAt: 'desc' },
      take: 4,
    }),
    prisma.workspaceNote.findMany({
      where: { ...noteWhere, type: 'knowledge', status: { not: 'archived' } },
      include: { createdBy: { select: publicUserSelect } },
    ...
    prisma.workspaceNote.count({ where: { ...noteWhere, type: 'note', status: { not: 'archived' } } }),
    prisma.workspaceTask.count({ where: { ...taskWhere, status: { not: 'done' } } }),
    prisma.workspaceTask.count({ where: { ...taskWhere, status: 'done', updatedAt: { gte: startOfDay, lt: endOfDay } } }),
    prisma.workspaceNote.count({ where: { ...noteWhere, type: 'knowledge', isPinned: true, status: { not: 'archived' } } }),
    prisma.workspaceTask.count({ where: { ...taskWhere, priority: 'critical', status: { not: 'done' } } }),
  ]);
  ...
  return {
    stats: {
      ...
      pinnedKnowledge: pinnedKnowledgeCount,
      criticalItems: criticalItemsCount,
    },
    recentNotes: notes.map(normalizeWorkspaceNote),
    pinnedNotes: pinnedNotes.map(normalizeWorkspaceNote),
    knowledge: knowledge.map(normalizeWorkspaceNote),
    openTasks: tasks.map(normalizeWorkspaceTask),
    templates: templates.map(normalizeWorkspaceTemplate),
    activity: activities.map(normalizeWorkspaceActivity),
  };
}

function workspaceTextFilter(query) {
...
async function listWorkspaceNotes(actor, filters = {}) {
  const prisma = requirePrisma();
  await ensureWorkspaceDemoContent(prisma, actor);

  const type = workspaceNoteTypes.has(filters.type) ? filters.type : 'note';
  const where = {
    AND: [
      workspaceVisibleWhere(actor),
      workspaceTextFilter(filters.search),
      { type },
...
async function getWorkspaceNote(actor, id) {
  const prisma = requirePrisma();
  const note = await prisma.workspaceNote.findFirst({
    where: { ...workspaceVisibleWhere(actor), id },
    include: { createdBy: { select: publicUserSelect } },
  });

  if (!note) {
    throw new ApiError(404, 'Workspace note not found.');
  }

  return normalizeWorkspaceNote(note);
}

function assertCanModifyWorkspaceEntity(actor, entity) {
  if (isWorkspaceAdmin(actor) || entity.createdById === actor.sub) return;
  throw new ApiError(403, 'You can only modify your own workspace content.');
}

function workspaceNoteData(payload, actorId, existing = null) {
  const title = String(payload.title || existing?.title || '').trim();
  if (!title) {
    throw new ApiError(400, 'Title is required.');
  }

  const type = workspaceNoteTypes.has(payload.type) ? payload.type : existing?.type || 'note';
  const status = workspaceNoteStatuses.has(payload.status) ? payload.status : existing?.status || 'active';
  const visibility = workspaceVisibility.has(payload.visibility) ? payload.visibility : existing?.visibility || 'private';

  return {
    title: title.slice(0, 220),
    content: normalizeWorkspaceContent(payload.content !== undefined ? payload.content : existing?.content),
    type,
    status,
    tags: normalizeWorkspaceTags(payload.tags !== undefined ? payload.tags : existing?.tags),
    category: payload.category !== undefined ? String(payload.category || '').trim() || null : existing?.category || null,
    isPinned: payload.isPinned !== undefined ? Boolean(payload.isPinned) : Boolean(existing?.isPinned),
    visibility,
    linkedMachineId: payload.linkedMachineId ? Number(payload.linkedMachineId) : existing?.linkedMachineId || null,
    linkedTechnicianId: payload.linkedTechnicianId || existing?.linkedTechnicianId || null,
    linkedReportId: payload.linkedReportId ? Number(payload.linkedReportId) : existing?.linkedReportId || null,
    updatedById: actorId,
  };
}

async function createWorkspaceNote(actor, payload) {
  const prisma = requirePrisma();
  const data = workspaceNoteData(payload, actor.sub);
  ...
  await logWorkspaceActivity(prisma, actor, `created ${note.type}`, note.type, note.id);
  return normalizeWorkspaceNote(note);
}

async function updateWorkspaceNote(actor, id, payload) {
  const prisma = requirePrisma();
  ...
  assertCanModifyWorkspaceEntity(actor, existing);

  const note = await prisma.workspaceNote.update({
    where: { id },
    data: workspaceNoteData(payload, actor.sub, existing),
    include: { createdBy: { select: publicUserSelect } },
  });
  await logWorkspaceActivity(prisma, actor, `updated ${note.type}`, note.type, note.id);
  return normalizeWorkspaceNote(note);
}

async function archiveWorkspaceNote(actor, id) {
  const prisma = requirePrisma();
  const existing = await prisma.workspaceNote.findFirst({
    where: { ...workspaceVisibleWhere(actor), id },
  });
  ...
  await logWorkspaceActivity(prisma, actor, `archived ${note.type}`, note.type, note.id);
  return normalizeWorkspaceNote(note);
}

function workspaceTaskData(payload, actorId, existing = null) {
  const title = String(payload.title || existing?.title || '').trim();
  if (!title) {
    throw new ApiError(400, 'Title is required.');
  }

  const status = workspaceTaskStatuses.has(payload.status) ? payload.status : existing?.status || 'todo';
  const priority = workspacePriorities.has(payload.priority) ? payload.priority : existing?.priority || 'medium';
  ...
  return {
    ...
    assignedToId: payload.assignedToId !== undefined ? payload.assignedToId || null : existing?.assignedToId || actorId,
    tags: normalizeWorkspaceTags(payload.tags !== undefined ? payload.tags : existing?.tags),
    linkedNoteId: payload.linkedNoteId || existing?.linkedNoteId || null,
    linkedMachineId: payload.linkedMachineId ? Number(payload.linkedMachineId) : existing?.linkedMachineId || null,
    linkedTechnicianId: payload.linkedTechnicianId || existing?.linkedTechnicianId || null,
    linkedReportId: payload.linkedReportId ? Number(payload.linkedReportId) : existing?.linkedReportId || null,
    updatedById: actorId,
...
async function listWorkspaceTasks(actor, filters = {}) {
  const prisma = requirePrisma();
  await ensureWorkspaceDemoContent(prisma, actor);

  const where = {
    ...workspaceTaskVisibleWhere(actor),
    ...(filters.search ? { title: { contains: String(filters.search), mode: 'insensitive' } } : {}),
  ...
  return tasks.map(normalizeWorkspaceTask);
}

async function createWorkspaceTask(actor, payload) {
  const prisma = requirePrisma();
  const task = await prisma.workspaceTask.create({
    data: {
      ...workspaceTaskData(payload, actor.sub),
      createdById: actor.sub,
      updatedById: null,
    },
    include: {
  ...
  await logWorkspaceActivity(prisma, actor, 'created task', 'task', task.id);
  return normalizeWorkspaceTask(task);
}

async function updateWorkspaceTask(actor, id, payload) {
  const prisma = requirePrisma();
  const existing = await prisma.workspaceTask.findFirst({
    where: { ...workspaceTaskVisibleWhere(actor), id },
  });
  if (!existing) {
    throw new ApiError(404, 'Workspace task not found.');
  }
  assertCanModifyWorkspaceEntity(actor, existing);

  const task = await prisma.workspaceTask.update({
    where: { id },
    data: workspaceTaskData(payload, actor.sub, existing),
    include: {
      createdBy: { select: publicUserSelect },
      assignedTo: { select: publicUserSelect },
    },
  });
  await logWorkspaceActivity(prisma, actor, `updated task to ${task.status}`, 'task', task.id);
  return normalizeWorkspaceTask(task);
}

async function deleteWorkspaceTask(actor, id) {
  const prisma = requirePrisma();
  const existing = await prisma.workspaceTask.findFirst({
    where: { ...workspaceTaskVisibleWhere(actor), id },
  });
  ...
  await logWorkspaceActivity(prisma, actor, 'deleted task', 'task', task.id);
  return normalizeWorkspaceTask(task);
}

async function listWorkspaceTemplates(actor) {
  const prisma = requirePrisma();
  await ensureWorkspaceDemoContent(prisma, actor);
...
function workspaceTemplateData(payload, actorId, existing = null) {
  const title = String(payload.title || existing?.title || '').trim();
  if (!title) {
    throw new ApiError(400, 'Title is required.');
  }
...
async function createWorkspaceTemplate(actor, payload) {
  const prisma = requirePrisma();
  const template = await prisma.workspaceTemplate.create({
    data: {
      ...workspaceTemplateData(payload, actor.sub),
      isSystem: false,
      createdById: actor.sub,
      updatedById: null,
    },
  });
  await logWorkspaceActivity(prisma, actor, 'created template', 'template', template.id);
  return normalizeWorkspaceTemplate(template);
}

async function updateWorkspaceTemplate(actor, id, payload) {
  const prisma = requirePrisma();
  const existing = await prisma.workspaceTemplate.findFirst({
    where: {
      ...
      OR: [
        { createdById: actor.sub },
        ...(isWorkspaceAdmin(actor) ? [{ isSystem: true }] : []),
      ],
    },
  });
  if (!existing) {
    throw new ApiError(404, 'Workspace template not found.');
  }

  const template = await prisma.workspaceTemplate.update({
    where: { id },
    data: workspaceTemplateData(payload, actor.sub, existing),
  });
  await logWorkspaceActivity(prisma, actor, 'updated template', 'template', template.id);
  return normalizeWorkspaceTemplate(template);
}

async function deleteWorkspaceTemplate(actor, id) {
  const prisma = requirePrisma();
  const existing = await prisma.workspaceTemplate.findFirst({
    where: {
      id,
  ...
  await logWorkspaceActivity(prisma, actor, 'deleted template', 'template', template.id);
  return normalizeWorkspaceTemplate(template);
}

async function listWorkspaceActivity(actor) {
  const prisma = requirePrisma();
  await ensureWorkspaceDemoContent(prisma, actor);
  const activities = await prisma.workspaceActivity.findMany({
    where: isWorkspaceAdmin(actor) ? {} : { userId: actor.sub },
    include: { user: { select: publicUserSelect } },
    orderBy: { createdAt: 'desc' },
    take: 30,
  });
  return activities.map(normalizeWorkspaceActivity);
}
